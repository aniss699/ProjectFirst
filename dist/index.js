...
import { z } from "zod";
var missionStatusEnum, currencyCodeEnum, urgencyLevelEnum, users, missions, bids, announcements, aiEvents, feedFeedback, feedSeen, favorites, insertMissionSchema, selectMissionSchema, insertBidSchema, insertFeedFeedbackSchema, insertFeedSeenSchema, insertFavoriteSchema;
...
// server/services/data-consistency-validator.ts
var DataConsistencyValidator = class {
  /**
   * Valide la cohérence Frontend → API
   */
  static validateFrontendToAPI(frontendData, apiData) {
    const errors = [];
    const warnings = [];
   ...
   */
  static validateAPIToDatabase(apiData, dbData) {
    const errors = [];
    const warnings = [];
   ...
   */
  static validateDatabaseToFeed(dbData, feedData) {
    const errors = [];
    const warnings = [];
   ...
   */
  static validateFullPipeline(frontendData, apiData, dbData, feedData) {
    const results = [
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    await db.delete(bids).where(eq2(bids.mission_id, missionIdInt));
